            ...
            case '/pizzas'://PUNTO 3 SOLO TIPO ENCARGADOS;
                ...
                if($tipoUsuario)
                {
                    if(isset($_POST['tipo']) && isset($_POST['precio']) && isset($_POST['stock']) && isset($_POST['sabor']) && isset($_FILES['foto']))
                    {
                        ...
                        if(!Pizza::BuscarPizza($_POST['tipo'], $_POST['sabor']))
                        {
                            ...
                            Img('imagenes/' . $_FILES['foto']['name'], 'imagenes/marcada.jpg');
                            GuardarTxt($pizza, 'pizzas.txt');
                            $datos = 'post pizzas ok';
                        }
                        else
                        {
                            $datos = 'Pizza ya existente';
                        }
                    }
                    else
                    {                        
    ...
    case 'GET':
        ...
        switch ($path_info) 
        {
            case '/pizzas'://PUNTO 4
                if($tipoUsuario)
                {
                    $datos = Pizza::MostrarPizzasEncargado();
                    $datos != '' ??  $datos = 'error al mostrar pizzas al encargado';

                }
                else
                {
                    $datos = Pizza::MostrarPizzasCliente();
                    $datos != '' ??  $datos = 'error al mostrar pizzas al cliente';
                }
                break;
            case '/ventas'://PUNTO 6
                if(isset($token))
                {
                    ...
                    {
                        //mostrar todas las ventas y cantidad
                        $datos = Venta::MontoEncargado();
                    }
                    else//user
                    {
                        //mostrar ventas del usuario
                        $datos = Venta::MontoCliente($auxUsu);
                    }
                }
                else
                {
                ...
                break;
            default:
                $datos = 'error al recibir reques method';
                break;
    }
    break;  
    default:
